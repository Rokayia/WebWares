...
<hr class="small-hr">
  ...
  <hr class="small-hr">
    ...
    <div class="partie-card-panier" v-if="commandes.length">
      <!-- Si le panier n'est pas vide, afficher les produits -->
      <div v-for="(prod, id) in commandes[0].produits" :key="id">
        <div v-for="product in produits" :key="product.id">
          <div v-if="prod.produitId == product.id">
            <div class="card-panier">
              <productCard class="produit-panier" :afficheMoq="false" :image="getImgUrl(product)" :titre="product.titre" :prix="product.prix" backgroundColor="red">  
                <div class="compteur">
                  <button class="btn-compteur" @click="prod.quantite-- " v-bind:disabled="prod.quantite === 1">-</button>
                  <h2 class="quantite-compteur">{{ prod.quantite }}</h2>
                  <button class="btn-compteur" @click="prod.quantite++">+</button>
                </div> 
              ...
              <div class="partie-prix">
                <button class="btn-supprimer" @click="removeProduct(prod)" label="Supprimer du panier">Supprimer</button>
...
export default {
  ...
  return (prod.quantite * product.prix).toFixed(2);
},
     totalProductTTC(prod, product) {
  // Taux de TVA 
  const tauxTVA = 20;
  // Calcul du ttc
  const totalTTC = (prod.quantite * product.prix * (1 + tauxTVA / 100)).toFixed(2);
  ...
  this.commandes.forEach(element => {
    total += element.produits.reduce((subtotal, prod) => subtotal + prod.quantite * this.produits.find(p => p.id === prod.produitId).prix, 0);
  });
...
totalBagTTC() {
  ...
  let total = 0;
  this.commandes.forEach(element => {
    total += element.produits.reduce((subtotal, prod) => {
      const produit = this.produits.find(p => p.id === prod.produitId);
      const totalProduitHT = prod.quantite * produit.prix;
 ...
 // Supprime un produit du panier
  ...
  computed: {
    ...
    ...mapState(['commandes', 'produits', 'currentProduct']),
